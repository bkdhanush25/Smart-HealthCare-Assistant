...
groq_llm = ChatGroq(
    model_name="llama3-70b-8192",from langchain.prompts import PromptTemplate
from langchain.chains import LLMChain
from langchain_groq import ChatGroq
from dotenv import load_dotenv
import os
...
Wet Sabzi, Dry Sabzi, Dal, Veg CuNon-Veg Curry, Egg Dish, Chicken Dish, Mutton Dish,
...
DO NOT include any explanations or preamble. ONLY return the Category output.
"""
)
...
# Prompt template for calculating total dish quantity
total_quantity_prompt = PromptTemplate(
    input_variables=["dish_name", "quantities_in_grams"],
    template=""" 
Given a dish name and the corresponding ingredient quantities in grams, calculate the total weight of the dish in grams.

If the dish name includes a quantity (e.g., "5kg of biryani" or "5 plates of biryani"), use that quantity directly without recalculating the ingredient amounts.

If the dish name does not include a quantity, calculate the total weight using the provided ingredient quantities in grams.

Dish Name: {dish_name}
Ingredient Quantities (in grams): {quantities_in_grams}

Total Weight (in grams):

DO NOT include any explanations or preamble. ONLY return the number output.
"""
)

# Chain for calculating total quantity
total_quantity_chain = LLMChain(llm=groq_llm, prompt=total_quantity_prompt)
...
def extract_ingredients_from_dish(dish_name):
    try:
        ...
        # Calculate total quantity using the separate chain
        total_quantity_response = total_quantity_chain.run(
            dish_name=dish_name, quantities_in_grams=quantities_in_grams
        )
        total_quantity = total_quantity_response
        return ingredients, quantities_in_grams, total_quantity
    except Exception:
        return [], {}, 0  # Return empty lists/dictionaries and 0 if there's an error
